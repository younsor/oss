...
public class HttpClient implements Runnable
{
    ...
    private boolean cleanAndRetryIfBroken(SelectionKey key, Request req)
    {
        ...
        if (req.isReuseConn && req.decoder.state == READ_INITIAL)
        {
            ...
            selector.wakeup();
            return true; // retry: re-open a connection to server, sent the
                         // request again
        }
        return false;
    }
    ...
    private void doRead(SelectionKey key, long now)
    {
        ...
        if (read == -1)
        { // read all, remote closed it cleanly
            if (!cleanAndRetryIfBroken(key, req))
            {
                req.finish();
            }
        ...
        if (job != null)
        {
            ...
            if (maxConnections == -1 || numConnections < maxConnections)
            {
                try
                {
                    ...
                    numConnections++;
                    // if connection is established immediatelly, should wait
                    // for write. Fix #98
    ...
    public void run()
    {
        while (running)
        {
            ...
            { // catch any exception (including OOM), print it: do not exits the
              // loop
